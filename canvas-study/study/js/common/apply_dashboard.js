define(["require","exports","getContext"],function (reuqire,exports,getContext) {
...
function drawTick(anlge,radius,cnt) {
   var tickWidth=cnt%4===0?TICK_WIDTH:TICK_WIDTH/2;
   context.beginPath();
   context.moveTo(circle.x+Math.cos(anlge)*(radius-tickWidth),
                  circle.y+Math.sin(anlge)*(radius-tickWidth));
   context.lineTo(circle.x+Math.cos(anlge)*radius,
                  circle.y+Math.sin(anlge)*radius);
...
function drawAnnotations() {
  ...
  context.font=ANNOTATIONS_TEXT_SIZE;
